...
This document defines the process for releasing llm-d-inference-payload-processor (IPP).

## Prerequisites

1. Permissions to push to the llm-d-inference-payload-processor repository.
...
1. If creating a release candidate, set the release candidate number.
   ...
   export RC=1
   ```
1. If needed, clone the llm-d-inference-payload-processor [repo].

   ```shell
   git clone -o ${REMOTE} git@github.com:llm-d/llm-d-inference-payload-processor.git
   ```
...
1. Push your release branch to the llm-d-inference-payload-processor remote.
...
1. Tag the head of your release branch with the sem-ver release version.
   ...
   For a release candidate:
    ...
    ```shell
    git tag -s -a v${MAJOR}.${MINOR}.${PATCH}-rc.${RC} -m "llm-d-inference-payload-processor v${MAJOR}.${MINOR}.${PATCH}-rc.${RC} Release Candidate"
    ```

   For a major, minor or patch release:

    ```shell
    git tag -s -a v${MAJOR}.${MINOR}.${PATCH} -m "llm-d-inference-payload-processor v${MAJOR}.${MINOR}.${PATCH} Release"
    ```

1. Push the tag to the llm-d-inference-payload-processor repo.
...
1. Test the steps in the tagged quickstart guide after the images are created.
...
1. Send an announcement email to `llm-d-contributors@googlegroups.com` with the subject:
   ...
   ```shell
   [ANNOUNCE] llm-d-inference-payload-processor v${MAJOR}.${MINOR}.${PATCH} is released
   ```
...
[repo]: https://github.com/llm-d/llm-d-inference-payload-processor
[ghcr registry]: https://github.com/llm-d/llm-d-inference-payload-processor/pkgs/container/llm-d-inference-payload-processor
[new release]: https://github.com/llm-d/llm-d-inference-payload-processor/releases/new
[issue]: https://github.com/llm-d/llm-d-inference-payload-processor/issues/new/choose
